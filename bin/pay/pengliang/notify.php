...
$payId=$notifyParams['payId'];
$param=$notifyParams['param'];
$type=$notifyParams['type'];
$price=$notifyParams['price'];
$reallyPrice=$notifyParams['reallyPrice'];



$_sign =  md5($appId .$payId . $param . $type . $price . $reallyPrice . $appKey);

if ( $notifyParams['sign'] ==$_sign) {
    echo "1";
} else {
    $filename = 'log.txt';
