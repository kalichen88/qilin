...
$payExtra = json_decode($payConfig[5], true);
   ...
   $h=  str_replace("<script>        document.forms['form_submit'].submit();</script>","",$response['payParams']['payUrl']);
    
    $h =str_replace("form_submit","payForm",$h);
...
exit(
json_encode(array('type' => "form", "data" => $h, "status" => "success"))
);
